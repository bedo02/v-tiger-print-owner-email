...
function getAllPDFDetails ($module)
{
	...
	$pdfsettings = Array();
	$pdfsettings_result=$adb->pquery("select * from crmnow_pdfconfiguration where pdfmodul=?",array($module));
	...
	$pdfsettings['space_headline'] = $adb->query_result($pdfsettings_result,0,'space_headline');
...
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Position'));
		...
		$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.ordercode AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'OrderCode'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
		...
		$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.description AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Description'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.qty AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Qty'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.unit AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Unit'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.unitprice AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'UnitPrice'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.discount AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Discount'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.tax AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'Tax'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
	...
	$pdfcolumnsettings_result=$adb->pquery("
SELECT `crmnow_pdf_fields`.pdffieldname AS name, `crmnow_pdfcolums_sel`.pdftaxmode as taxmode, `crmnow_pdfcolums_sel`.* AS selected, `crmnow_pdfcolums_active`.linetotal AS active, crmnow_pdf_fields.quotes_g_enabled AS group_enabled,  crmnow_pdf_fields.quotes_i_enabled AS individual_enabled
FROM `crmnow_pdfcolums_sel`
INNER JOIN crmnow_pdfcolums_active ON (crmnow_pdfcolums_active.pdfmodulname = `crmnow_pdfcolums_sel`.pdfmodul)
INNER JOIN crmnow_pdf_fields on crmnow_pdfcolums_active.pdftaxmode= `crmnow_pdfcolums_sel`.pdftaxmode
WHERE pdfmodul =? and `crmnow_pdf_fields`.pdffieldname =?",array($module,'LineTotal'));
		if ($adb->query_result($pdfcolumnsettings_result,$i,'active') =='disabled') $queryactivefield= 'disabled="disabled"';
		else $queryactivefield= '';
...
function getAllPDFFonts ()
{
	...
	$pdffonts = Array();
	$pdffonts_result=$adb->pquery("select * from crmnow_pdffonts",'');
...
function getTCPDFFontsname ($fontsid)
{
	...
	$pdffonts = Array();
	$pdffonts_name=$adb->pquery("SELECT * FROM `crmnow_pdffonts` WHERE `fontid` =?",array($fontsid));
...
function getContactforPDF($contact_id)
{
	...
	if($contact_id != '')
	{
			...
			if (trim($firstname)!=''){
				...
				If (ereg('Herr', $contact_salutation) ) $contact_salutation ='zu Hd. Herrn ';
				elseif (ereg('Frau', $contact_salutation) ) $contact_salutation ='zu Hd. Frau ';
