...
function createpdffile ($idnumber,$purpose='', $path='',$current_id='') {
	...
	//select language file
		switch($pdf_config_details['pdflang'])
		{
			case "en_us":
				...
				break;
			case "de_de":
				include("modules/Invoice/language/de_de.lang.pdf.php");
				$language = "DE";
				break;
			case "pl_pl":
				include("modules/Invoice/language/pl_pl.lang.pdf.php");
				$language = "PL";
				break;
			default:
				include("modules/Invoice/language/en_us.lang.pdf.php");
	...
	elseif ($purpose=='print'){
		$pdf->Output($export_org.'_'.$FACTURE.'_'.$date_issued.'.pdf','D'); 
		exit;
	}
	elseif ($purpose=='send'){
	// send pdf with mail
		...
		{
			case "EN":
				$pdf->Output('storage/Invoice.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
			case "DE":
				$pdf->Output('storage/Rechnung.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
			case "PL":
				$pdf->Output('storage/Faktura.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
		}
		return;
		exit;
