...
//decide whether this an invoice or credit note
if ($invoice_status=='Credit Invoice') $pdf->Cell($pdf->GetStringWidth($pdf_strings['CREDIT']),$pdf->getFontSize(), $pdf_strings['CREDIT'],0,1);
else $pdf->Cell($pdf->GetStringWidth($pdf_strings['FACTURE']),$pdf->getFontSize(), $pdf_strings['FACTURE'],0,1); 
...
// the maximum width of all columns must be 180
...
If ($summaryradio == 'true') { 
	...
	$data = $pdf_strings['VAR_TOTAL']." (in $currency_symbol):";
	$pdf->SetXY( 105 , $line_y_location );
...
$pdf->Image('test/upload/Peciatka_200x116.jpg', $x='140', $current_y_location+30, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=600, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false);
