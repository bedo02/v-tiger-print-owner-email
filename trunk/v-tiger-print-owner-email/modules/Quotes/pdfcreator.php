...
function createpdffile ($idnumber,$purpose='', $path='',$current_id='') {
	...
	//$result = $adb->query($sql);
	//$currency_symbol = $adb->query_result($result,0,'currency_code');
	//internal number
	$id = $idnumber;
	...
	// send pdf with mail
		switch($language)
		{
			case "EN":
				...
				break;
			case "DE":
				$pdf->Output('storage/Angebot.pdf','F'); //added file name to make it work in IE, also forces the download giving the user the option to save
				break;
			case "PL":
