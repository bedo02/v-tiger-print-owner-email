...
if ($_REQUEST['fld_module'] != '') $entrymodule = $_REQUEST['fld_module'];
else $entrymodule = 'Settings';
if ($entrymodule =='Settings') $action= 'PDFConfig';
else $action ='DetailView';
if (isset ($_REQUEST['parenttabstart'])) $parenttabstart = $_REQUEST['parenttabstart'];
else $parenttabstart = '';

foreach ($pdfmodule as $modulename) {
	if ($entrymodule!='Settings') {
		//get PDF permissions from Settings
		$pdfsettings_query="select * from crmnow_pdfsettings where pdfmodul='".$modulename."'";
	...
	for($i=0; $i<$noofrows; $i++) {
		$update_query = "update crmnow_pdfcolums_sel set ".$changablecolumns[$i]."=? where pdfmodul=? ";
		...
		elseif ($requestarry[1] =='group' or $requestarry[1]='individual'){
			...
			if ($modulename==$requestarry[0]) {
				...
				if ($pdfpermission[$modulename][$requestarry[2]] != 1){	
					...
					$update_query = "update crmnow_pdfcolums_sel set ".strtolower($requestarry[2])."=? where pdfmodul=? and pdftaxmode=?";
	...
	if ($entrymodule=='Settings'){
		...
		foreach ($fieldlist as $fieldvalue) {
			...
			$update_query = "update crmnow_pdfsettings set pdfeditable=? where pdffieldname=? and pdfmodul=?";
	...
	foreach ($fieldlist as $fieldvalue) {
		...
		if ($pdfpermission[$modulename][$fieldvalue] != 1){	
			$update_query = "update crmnow_pdfconfiguration set ".$fieldvalue."=? where pdfmodul=?";
...
function getPDFConfigList($fld_module)
{
	...
	$pdfconfig_query="select * from crmnow_pdfconfiguration where pdfmodul=?";
...
var module = '<?php echo $entrymodule; ?>';
if (module=='Settings'){
	window.document.location.href = 'index.php?action=<?php echo $action; ?>&module=<?php echo $entrymodule; ?>&parenttab=<?php echo $parenttabstart; ?>&displaymodul=<?php echo $entrymodule; ?>&frommodule=<?php echo $entrymodule; ?>&record=<?php echo $entryid; ?>';
}
else{
/**  we need to refresh the detail view to load the new values for the pdf output (mail) */
opener.document.location.href = 'index.php?action=<?php echo $action; ?>&module=<?php echo $entrymodule; ?>&parenttab=<?php echo $parenttabstart; ?>&displaymodul=<?php echo $entrymodule; ?>&frommodule=<?php echo $entrymodule; ?>&record=<?php echo $entryid; ?>';
self.close();
}
</script>
</html>
