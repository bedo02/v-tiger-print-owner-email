		...
		if($fieldname == $focus->list_link_field)
		{
			if($mode == "search")
			{
				...
				elseif($popuptype == "inventory_prod")
				{
					...
					//crm-now: bugfix to get product description in popup menue saved
					$product_des =  str_replace(chr(34), "&quot;", $description);
				...
				elseif($popuptype == "inventory_service")
				{
					...
					//crm-now: bugfix to get product description in popup menue saved
					$product_des =  str_replace(chr(34), "&quot;", $description);
					$slashes_desc = htmlspecialchars($product_des,ENT_QUOTES,$default_charset);
					...
					require_once('include/Zend/Json.php');
					$prod_arr = Zend_Json::encode($tmp_arr);
					$value = '<a href="javascript:window.close();" id=\'popup_product_'.$entity_id.'\' onclick=\'set_return_inventory("'.$entity_id.'", "'.nl2br($slashes_temp_val).'", "'.$unitprice.'", "'.$tax_str.'","'.$row_id.'","'.$slashes_desc.'","'.$order_code.'");\'  vt_prod_arr=\''.$prod_arr.'\' >'.$temp_val.'</a>';
